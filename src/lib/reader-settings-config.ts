...
export const READER_SETTINGS_STORAGE_KEY = "story-forge-reader-settings";

export function readStorage(): ReaderSettings {
...
export function writeStorage(s: ReaderSettings): void {
  try {
    window.localStorage.setItem(READER_SETTINGS_STORAGE_KEY, JSON.stringify(s));
  } catch {
